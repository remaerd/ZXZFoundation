...
#import "SCSwipeableTableViewController.h"

@interface SCSwipeableTableCell()
...
@implementation SCSwipeableTableCell
	...
	if (pan.state == UIGestureRecognizerStateBegan) self.originalPointX = x;
		...
		[UIView animateWithDuration:0.2 animations:^{
			if (failed) [self.content setTransform:CGAffineTransformIdentity];
				...
				[((SCSwipeableTableViewController*)self.superview).delegate tableViewCell:self didTriggerButtonDirection:self.direction index:self.currentIndex];
			}
